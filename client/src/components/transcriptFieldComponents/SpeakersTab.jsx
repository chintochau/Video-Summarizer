...
const SpeakersTab = (params) => {
  ...
  const { utterances, speakers, setSpeakers, transcriptId } =
...
const UtteranceBox = ({ utterance, onClick, currentPlayTime, speakers }) => {
  ...
  const isCurrent =
    currentPlayTime > utterance.start / 1000 && currentPlayTime < end;
  ...
  useEffect(() => {
    const speaker = speakers.find(
      (speaker) => speaker.id === utterance.speaker
    );
    if (speaker) {
  ...
  return (
    <div className=" border-b py-1">
      <div
        className={cn(
          "flex gap-2 cursor-pointer hover:outline outline-blue-400 rounded-md mx-2 my-1 outline-1 p-1 mb-2",
          isCurrent && "bg-blue-100"
        )}
        onClick={() => onClick(time)}
      >
        <div className=" w-1/5  text-right mr-4 font-semibold">
          <div style={{ color: color }}>{speakerName}</div>
          <div
