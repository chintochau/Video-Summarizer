...
export const saveEmbeddings = async ({ video, userId, embeddings }) => {
    try {
        ...
        const defaultEmbeddingCollection = await EmbeddingCollection.findOne({ userId, name: "default" });
        ...
        const updatedEmbeddingCollection = await EmbeddingCollection.findOne({ userId, name: "default" });
...
export const removeEmbeddings = async ({ userId, videoSourceId }) => {
    try {
        ...
        await EmbeddingCollection.updateMany({ _id: { $in: collectionIds } }, { $pull: { videos: videoId, embeddings: { $in: embeddingIds } } });
    }
    catch (error) {
...
export const getEmbeddingCollection = async (userId, page = 1, limit = 10) => {
    try {
        const userCollection = await EmbeddingCollection.findOne({ userId, name: "default" }).populate({ path: "videos", options: { skip: (page - 1) * limit, limit } });
